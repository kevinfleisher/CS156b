...
# We load in the Google vision transformer model weights, which have 
# been already fine tuned on the NIH chest x-ray dataset, and now 
# fine tune these weights four separate times and the model weights 
# are saved each time. 
# Each run corresponds to the model's ability to predict one of the 
# discrete labels in the data set.
...
dir_path = './batched_Datasets_3_custom_channels/'
...
for i, name in enumerate(OVR_names):
    ...
    # Load in the NIH fine tuned model weights!
    # model_name_or_path = "google/vit-base-patch16-224-in21k"
    model_name_or_path = "./models/CS156b/vit_nih_3_custom_channels"
    ...
    training_args = TrainingArguments(
        output_dir=f"./models/CS156b/vit_nih_to_chexpert_3_custom_channels_4_labels_OVR_{i+1}",
